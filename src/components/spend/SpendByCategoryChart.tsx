  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div style={tooltipStyle} className="p-2.5">
        ...
        <p className="text-xs text-slate-300">{formatCurrency(d.value)}</p>
        <p className="text-xs text-slate-300">{d.pct.toFixed(1)}% of total</p>
        <p className="text-xs text-slate-300">{d.count} transactions</p>
      </div>
    );
  };
